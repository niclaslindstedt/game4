...
/// The shipped default: 16:9 and the desktop reference viewport the UI is
/// reviewed at, so a first launch is a sensible landscape window rather than
/// whatever the window manager would pick.
...
/// viewport with room for the window chrome — below it the HUD's corners and
/// the minimap start colliding.
