...
Six sleds ship — a crossover, a trail sled, a mountain sled, a snocross racer, a turbo tourer and a work sled, each a real class of machine traced off its photographs, an answer to a kind of snow, dressed in four liveries and with no real brand behind it — with their rider on the pegs. Every hill, every tree and every sled is written in code; the game ships no asset files (unless a build asks for the machines and riders modelled in Blender — `VITE_MODEL_SLEDS=1`, `VITE_MODEL_RIDERS=1`), and every sound is synthesized.
...
| `make models`           | Every sled and the rider modelled at game quality, for a build that draws them (`VITE_MODEL_SLEDS=1`, `VITE_MODEL_RIDERS=1` — the environment or the root `.env`); needs Blender                                                                                                                                                                                                         |
