...
Today there are two. **v1** is the generator the campaign was curated on, kept alive by two
traits: `fewerJumps` (`JumpTraits`, read through `jumpsOf`): no rollers, no cliffs, one to three
kickers on the loop at least 450 m apart and five to ten off it; and `scatteredForest`, read in
`forest.ts` and nowhere else: its woods grow neither R14's clumps nor its lanes, at the whole
density. **v3** is the rules as written — R3's rollers, R22's cliffs, three to eight kickers on the
loop 220 m apart, eight to sixteen off it, and the woods in clumps with lanes through them — and
is what every free ride, race off a link and lab builds. (v2, the rollers and the cliffs without
the new woods, was current until v3, named by no campaign map, and went with it.) The analysis
