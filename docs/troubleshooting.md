...
- **No sound.** A browser makes no sound until the page has been touched — a tap or a key after it loads unlocks it. Check the SOUND switch on the front door or the pause card, and on an iPhone the ringer switch (the browser respects it; the store app does not).
