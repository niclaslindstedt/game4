...
- **No cloud save.** The sibling jet-ski game's store app syncs a save through iCloud; this game's website has no save to sync, and a shell may only bridge to something the website already does. The SCREENSHOT bridge is carried: both shells press the game's own shutter by the `shot` word on `sh-shell-command` — the desktop app from View ▸ Take Screenshot (⇧⌘S), the store app when the phone takes a screenshot of its own (`native/src/screen-capture.ts`, `SHOT_COMMAND`), so the hardware's picture is filed in the game's gallery too.
