...
| `menu=root`   | Open on the front door rather than the attract card; `menu=sled` the sled card, `menu=start` the free ride's start card, `menu=options` OPTIONS, `menu=keys` its KEYS page, `menu=gallery` the GALLERY, `menu=campaign` the campaign card, `menu=levels` the level card (with `mode=trial` for a TIME TRIAL's). |
