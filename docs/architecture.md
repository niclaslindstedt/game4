...
- **What is remembered** — `game/settings.ts`: the camera rung, the sound switch and every OPTIONS row, merged field by field (`mergeSettings`). OPTIONS (`menu-options.tsx`) and its KEYS page (`menu-keys.tsx`) are the `menu` surface's pages, built from `menu-knobs.tsx`'s rows.
