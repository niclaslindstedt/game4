...
| What a run is PLAYING BY: the modes and their rules (`MODE_RULES`: the race, the time trial), the open rules a measurement rides | `engine/game/defs/modes.ts` (`RunRules`, `GameMode`, `MODE_RULES`, `raceRules`, `timeTrialRules`, `openRules`, `TIME_TRIAL`), `GameState.rules`, `createGame`'s `mode` | `engine-system` |
...
- **The heading to the owed checkpoint** — `bearingToNext` in `engine/game/course.ts`, for the HUD's arrow and the bot alike. **Where a reset stands the sled** — `resetPose` beside it: on the track past the last checkpoint TAKEN, facing along it; `standSled` is how anything puts a sled down at rest.
