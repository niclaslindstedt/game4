...
**A developer page is built too:** let out by holding the front door's TITLE for seven seconds (the DEVELOPER chip is the receipt): instruments over the picture — the frame rate, the frame's cost, the physics' readouts (every probe's load, sink and travel, the packed share), the trail maps in the corner, the engine's debug log and a free camera — with a REPRO line at the overlay's foot (seed, mode, sled, time, camera, sky and the sled's pose, as a URL `?start=race&seed=&t=&pose=` reads back); UNLOCKS, the campaign's board set by hand; and the BENCHMARK — a pinned race through the woods and over a kicker, the whole field ridden by the bot, drawn as fast as the machine can and scored as an index (100 is real time), kept in a history, and written out by COPY DEBUG REPORT (`make bench` runs it headlessly) (`debug-tools`).
...
`sim`, `ride`, `level`, `analyze`, `rate`, `difficulty` and `routes` are pure Node — no build, no browser, seconds. `audition` writes a page in pure Node; its `--meter` drives that page in Chromium. `screenshots`, `profile` and `bench` drive the BUILT SITE, so **`make build` first, every time**: a stale dist photographs the last change rather than this one. `world` and `sky` build their own one-off bundles from harness pages (`pwa/world-preview.html`, `pwa/sky-preview.html`) and need no `make build`. The browser-driven ones need `npm i --no-save playwright-core`; in Claude web sessions Chromium is preinstalled — prefix them with `CHROMIUM_PATH=/opt/pw-browsers/chromium`.
...
| The rule book (R1–R20) | `engine/mapgen/rules.ts`, mirrored verbatim in `docs/level-generator.md` | `mapgen-improvement` |
...
| THE DEVELOPER PAGE: the title's hold, the instruments (frame rate and cost, physics, trail map, engine log, free camera), the overlay and its REPRO line, UNLOCKS | `pwa/src/game/menu-hold.ts`, `menu-dev.tsx`, `dev-tools.ts`, `dev-app.tsx` (the app's half), `debug-readout.ts` (DOM-free), `debug-hud.tsx`, `frame-rate.ts`, `free-fly.ts`, `trail-overlay.ts`, `campaign-unlocks.ts`, `menu-unlocks.tsx`; `pwa/src/dev.css` | `debug-tools`, `menu-system` |
| THE BENCHMARK: the pinned race, the pump and its fence, the index, the report and its `FrameCost` slices, the scene walked, the history, the card | `pwa/src/game/benchmark-plan.ts`, `benchmark.ts`, `benchmark-index.ts`, `benchmark-report.ts`, `scene-tally.ts`, `benchmark-history.ts`, `bench-run.ts`, `menu-bench.tsx`; `scripts/benchmark.mjs` | `debug-tools` |
...
- **WHAT THE RULES ARE** — `engine/mapgen/rules.ts`: R1–R20 stated once in the header as prose and every number in `LEVEL_RULES`. The generator BUILDS to them, `engine/analysis/` HOLDS the finished map to them, the tests assert them, and `docs/level-generator.md` carries the prose verbatim.
...
- **Which surface is up** — `Shell` in `pwa/src/game/shell.ts` (`splash | menu | loading | pause | run | replay | bench`), and the predicates beside it (`watching` is the replay's). **The snow never stops behind a card — except the pause card**, the one card `simulates` says no to; the `bench` surface is the other no, because the benchmark's pump turns the snow itself (`appDraws`).
...
- **The rule book has a mirror.** `engine/mapgen/rules.ts` states R1–R20 once in its header; `docs/level-generator.md` carries the same prose VERBATIM. `tests/docs_rules_test.ts` reads the ids off the code, so a new rule fails the test until its mirror lands.
...
- **`debug-tools`** — the developer page behind the title's hold: the instruments over the picture, the free camera, the REPRO line and the URL that reads it back, UNLOCKS, and the BENCHMARK — its plan, its report, its history, `make bench`.
...
**Reserved, not written** (see `.agents/skills/README.md`): soundtrack, tricks, replay, store-listing, store-shots. A lesson about one of those subjects waits, scoped, in the nearest existing skill until its subject is built — and the sibling repos' skill of the same name is where it starts.
