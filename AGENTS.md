...
- **What the game remembers** — `pwa/src/game/settings.ts` (the camera, the sound switch, and every OPTIONS row: the faders, the picture, the keys, the thumbs, the assist), and `mergeSettings` is the one place a stored blob becomes settings this build offers. `mixOf` is the one place the master and the switch fold into the two faders the bus is handed.
