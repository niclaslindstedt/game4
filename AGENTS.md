...
| The rule book (R1–R19) | `engine/mapgen/rules.ts`, mirrored verbatim in `docs/level-generator.md` | `mapgen-improvement` |
...
- **WHAT THE RULES ARE** — `engine/mapgen/rules.ts`: R1–R19 stated once in the header as prose and every number in `LEVEL_RULES`. The generator BUILDS to them, `engine/analysis/` HOLDS the finished map to them, the tests assert them, and `docs/level-generator.md` carries the prose verbatim.
...
- **The rule book has a mirror.** `engine/mapgen/rules.ts` states R1–R19 once in its header; `docs/level-generator.md` carries the same prose VERBATIM. `tests/docs_rules_test.ts` reads the ids off the code, so a new rule fails the test until its mirror lands.
