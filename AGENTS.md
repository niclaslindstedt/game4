...
**This repository is a VERTICAL SLICE.** The engine — the world generator, the sled on the snow, the course, the race, the bot — is built and green, and so is the first playable cut of the app around it: ONE generated map per seed (no biomes — one nature: a basin of snowy hills ringed by mountain flanks, forests of snow-loaded conifers, open powder meadows, kickers on the hilltops), a closed-loop packed-snow race track graded into it with crests on it that are jumps, the grid standing out in the powder beside it; ONE sled with its rider; RACE mode only — the player and three bot rivals, three laps, checkpoints in order; a clear winter sky whose sun moves an hour every ten minutes of riding from the seed's hour, on the seed's day, at its latitude; a snow shader (the glitter, the blue in the shadows, the groomed track's corduroy) and snow that DEFORMS — every sled's skis and tread stamped into a world-space trail map, so the furrows persist; the spray; a ladder of five cameras; the synthesized sound (the engine and the belt, the hiss and the powder, the wind, every landing); a HUD (speed, revs, position, lap and checkpoint count, the race clock, the split, the air clock, the missed-checkpoint arrow, the news column, the finish plate), keyboard and touch; and the shell — an attract card, a front door with one RACE tile, a loading card, a pause card. Around THAT, both platform shells are built: `tauri/`, the desktop app, and `native/`, the store app. **NOT BUILT, and with no placeholder file to build into:** an options page, a developer page, a campaign, other modes, weather, cloud, night, a minimap, replays and ghosts, screenshots and a gallery, damage, tricks, music, a second sled. Do not describe any of them as a feature, and start any of them with `engine-system` — and the sibling repos' answer (below).
...
- **What the game remembers** — `pwa/src/game/settings.ts` (the camera and the sound), and `mergeSettings` is the one place a stored blob becomes settings this build offers. **What the URL says** — `pwa/src/game/url-params.ts`, and `scripts/screenshot.mjs` drives the built site through exactly that.
...
- Runner: vitest via `make test`; config in `vitest.config.ts` (alias `@engine` → `engine/index.ts`). No DOM, no browser — engine tests, plus the DOM-free app modules (`input-model.ts`, `snapshot.ts`, `run-news.ts`, `shell.ts`, `url-params.ts`, `settings.ts`, `camera-rigs.ts`, `rider-pose.ts`, `sky.ts`, `trail-stamp.ts`, the audio's vocabulary, bank, route and beds under a recording synth).
...
- **The sled is drawn off the physics' spec.** `pwa/src/game/sled-body.ts` builds the machine in the engine's body frame from `SLED` (the stance, the tread's run, the CoG height), and `rider-pose.ts`'s `MOUNTS` are where the drawn grips and boards are. Move the spec and the drawing follows; move the bars or the boards in the drawing and move `MOUNTS` with them.
...
- **`menu-system`** — the shell around a race: the attract card, the front door, the loading card, the pause card, the settings and the URL.
