...
| THE WEATHER a map is dealt (R18) and its evening; a sky chosen by hand (`withSky`); the moon (`moonAtRun`); the wind as a pure field (`windAt`) | `engine/mapgen/weather.ts`, `engine/game/clock.ts`, `engine/game/wind.ts` | `atmosphere`, `mapgen-improvement` |
