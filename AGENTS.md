...
**This repository is a VERTICAL SLICE.** The engine — the world generator, the sled on the snow, the course, the race, the bot — is built and green, and so is the first playable cut of the app around it: ONE generated map per seed (in the boreal forest unless a REGION is asked for — below: a basin of snowy hills ringed by mountain flanks, forests of snow-loaded conifers, open powder meadows, kickers on the hilltops), a closed-loop packed-snow race track graded into it with crests on it that are jumps, drifts of fresh snow across it and the plough's berms along both edges, the grid standing on it behind the start line; FOUR sleds in the engine's catalog — trail, crossover, mountain and cross, each an answer to a kind of snow, the player's picked on a SLED CARD (the machine turning on its stand, drawn off its own spec, with a spec sheet beside it) and the rivals dealt one each — with their rider; THREE MODES — the RACE (the player and three bot rivals, three laps, checkpoints in order), the TIME TRIAL (the same map alone under the lights, three laps or one, against the RECORD BOOK — the best time per seed, sled, mode and laps, with the split's gap to it at every checkpoint — and the GHOST of the run that set it, kept as the controls that rode it and ridden again beside the player, see-through and trail-less) and the FREE RIDE (the whole map and nobody on it, no lights, no laps, the checkpoints counting nothing and no record kept, on a day and a depth of snow of the rider's own); a winter sky whose sun moves an hour every ten minutes of riding from the seed's hour, on the seed's day, at its latitude, under one of six WEATHERS the seed deals (R19: clear, fair cumulus, high cloud, the flat light of an overcast, falling snow from light to a blizzard with spindrift off the crests, a valley fog), and on a quarter of the maps from sunset into the NIGHT — the moon and the stars, every sled's headlamp on the snow and its taillight; a snow shader (the glitter, the blue in the shadows, the groomed track's corduroy, the berms' clods) and snow that DEFORMS — every sled's skis and tread stamped into a world-space trail map, so the furrows persist; the spray; a ladder of five cameras; the synthesized sound (the engine and the belt, the hiss and the powder, the wind, every landing); a HUD (speed, revs, position, lap and checkpoint count, the race clock, the split, the air clock, the missed-checkpoint arrow, the news column, the finish plate, and a heading-up MINIMAP of the loop, the field and the checkpoint owed), keyboard and touch; and the shell — an attract card, a front door with the CAMPAIGN as its lit tile and a RACE tile and a TIME TRIAL tile (each on a pinned map picked on a level card), each opening the sled card, and a FREE RIDE tile that opens a START CARD first (the map's seed with a chart of it — the loop, the grid and every kicker — tapped to start anywhere, the date, the hour, the snow's depth), OPTIONS (the picture as eight ladders applied live over the race, with a first-visit probe that picks the rung this machine can hold; three sound faders; the keys rebound; the thumbs' side, travel and lean; how much the sled helps), a loading card, a pause card, and the REPLAY — every race, time trial and tricks run recorded as the controls that rode it, watched back from the finish plate or the pause card with the field ridden again beside it, on a BROADCAST camera planted at the edge of the wood that cuts to the moments the run marked and runs them in slow motion; and the SHUTTER — ENTER (or the desktop menu bar's row, or the phone's own screenshot in the store app) photographs the race with the HUD rasterized in and the mark stamped in the corner, H takes the readouts down for a clean frame, and a GALLERY chip on the front door opens the roll of the last forty (IndexedDB) to flip through, share, copy, save or delete. Around THAT, both platform shells are built: `tauri/`, the desktop app, and `native/`, the store app. **NOT BUILT, and with no placeholder file to build into:** a developer page, other modes, music. Do not describe any of them as a feature, and start any of them with `engine-system` — and the sibling repos' answer (below).

**The campaign is built too:** the front door's lit tile opens THREE SHELVES of six pinned maps (the foothills, the timberline, the summit — a race, a time trial, two races, a time trial and a race), each a seed on a generator VERSION with a digest the suite holds it to, ridden for points against the field (a podium on a race, a medal on a trial opens the next map; the shelf's table opens the next shelf). A RACE and a TIME TRIAL off the front door ride those same maps, picked on a level card out of the shelves the campaign has opened, so a record is a record round a loop anyone can ride; a seed of one's own is the free ride's and a link's (`campaign`, `level-rating`).

**Crashes are built too:** a trunk met hard, a nose-in landing or a rollover at speed throws the rider off onto a body of his own that tumbles in the snow until the reset stands them back up; a sled bogged in deep powder trenches itself in and is rocked back out (the lean and the bars); and, behind an OPTIONS switch that is off by default, a blow bends a ski or hurts the suspension and the sled rides it for the rest of the race, read off a HUD instrument (`crash`).
...
**Regions are built too:** the same seed can be raised in four kinds of snow country (R21, `engine/mapgen/regions.ts` — a kind of country, never a place): the BOREAL forest (today's map, and every map nobody asked a region of — its row is all ones, so no seed and no pinned campaign map moved), the high ALPINE (steep rough country, deep bowls, tall flanks, woods only in the hollows, wind crust on the crests and rock on the steep faces), the TUNDRA plateau (flat and open, a crust carved into sastrugi, the far north) and the BIRCH valley (pale birches and a frozen river across the basin — a road of ice with little grip on it), each with its own look and a whole-frame GRADE; chosen on the free ride's start card (COUNTRY) or a `?region=` link. The campaign is boreal throughout (`add-region`).
...
| A REGION (R21): its row, its surface, its look, its grade | `analyze COUNT=16 REGION=id`, `level REGION=id`, `sim REGION=id` (and the boreal's, unchanged), `world REGION=id`, `screenshots ARGS="--region id"`, `profile ARGS="--region id"` | `add-region` |
...
- **`make world SEED=38`** rides one seed with the bot through the game's own renderer and photographs named views (`spawn`, `powder`, `lookback`, `furrow`, `track`, `hood`, `bars`, `far`, `jump`, `landing`, `vista`, `forest`, `approach-140` … `approach-40`, `orbit`) — ONE continuous run, so the trails in the pictures are the trails that run cut. It exists because a screenshot of the app is one frame of one camera, and the furrows, the float and the landing are moments a still has to be steered to.
...
The root suite may import from a shell in exactly one place: the pure, import-free seam modules named in `tests/imports_test.ts`'s `SHELL_SEAM` (`native/src/injected.ts`, `navigation.ts`, `rumble.ts`, `cloud-ask.ts`). A seam module that grows an import fails that test. The desktop shell's Rust is held the other way, as TEXT (`tests/tauri_test.ts`).
...
| `pwa/src/game/campaign-routes.ts` — the loop behind each campaign box | `make routes` | `tests/generator_version_test.ts` rebuilds every pinned map and compares its line |
...
| THE WIPEOUT: what puts the rider off, his body thrown and tumbling, when the reset stands them up | `engine/game/crash.ts`, `TUNING.crash`, `SledState.thrown` | `crash` |
| STUCK IN DEEP POWDER: the trench a bogged tread digs, and rocking it out | `engine/game/trench.ts`, `TUNING.trench` | `crash`, `sled-physics` |
| DAMAGE (a run option, off by default): what a blow bends and how a bent sled rides; its HUD instrument | `engine/game/damage.ts`, `TUNING.damage`, `GameState.damage`; `pwa/src/game/hud-damage.tsx` | `crash`, `hud-and-menus` |
...
| What a run is PLAYING BY: the modes and their rules (`MODE_RULES`: the race, the time trial, the free ride with its `course` off), the open rules a measurement rides | `engine/game/defs/modes.ts` (`RunRules`, `GameMode`, `MODE_RULES`, `raceRules`, `timeTrialRules`, `freeRules`, `openRules`, `TIME_TRIAL`), `GameState.rules`, `createGame`'s `mode` | `engine-system` |
...
| A FREE RIDE stood up: where it starts, on which day | `freeSpawn` in `engine/game/course.ts`, `withDay` / `freeHours` in `engine/mapgen/sun.ts`, `createGame`'s `mode: "free"` / `spawn` / `day` / `snowDepth` | `engine-system` |
...
| THE TRICKS: the strokes (the pump, the twirl) and the poses; the score — the air, the turns, the combo, the landing it is judged on | `engine/game/strokes.ts`, `engine/game/tricks.ts`, `TUNING.tricks` (stated in `defs/tricks.ts`), `GameState.tricks`, `tricksRules` / `TRICKS_RUN` in `defs/modes.ts` | `engine-system`, `sled-physics` |
| THE TRICK FIELD (R20): graded kickers on the loop, laid only for a tricks run | `engine/mapgen/trick-field.ts`, `GenerateOptions.tricks`, `Kicker.trick`; held by `engine/analysis/trick-field.ts` | `mapgen-improvement` |
...
| The rule book (R1–R21) | `engine/mapgen/rules.ts`, mirrored verbatim in `docs/level-generator.md` | `mapgen-improvement` |
| THE REGIONS (R21): what a kind of snow country IS — the relief, the woods and what grows, the kickers, the sun's bands; its crust and its frozen river, folded into the packed field clear of the loop; the ice's grip | `engine/mapgen/regions.ts` (`regionOf`), `surface.ts`, `Level.crust` / `.ice` / `iceAt`, `onIce` in `engine/game/snow.ts` | `add-region`, `mapgen-improvement` |
| What a region LOOKS like, and the grade over the whole frame | `pwa/src/game/region-look.ts`, `colour-grade.ts` (three-free), `grade-pass.ts`, `region-picture.ts` (the one place a frame reaches the canvas) | `add-region`, `snow-look`, `atmosphere` |
| How a map is scored for DEFECTS | `engine/analysis/` | `mapgen-improvement` |
| How HARD a map is, and what kind of hard: the eight axes, the index, the ladder scorer | `engine/rating/index.ts` (`rateLevel`, `rateLadder`, `RATING`) | `level-rating` |
| WHICH GENERATOR built a map: the version registry, its traits, the digest | `engine/mapgen/versions.ts`, `digest.ts` (`levelDigest`); `GenerateOptions.version`, `Level.version` | `campaign`, `mapgen-improvement` |
| THE CAMPAIGN'S MAPS: the pinned seeds, six to a shelf — their versions, digests, days and medals | `pwa/src/game/campaign-levels.ts` — curated with `make rate CAMPAIGN=1` | `campaign` |
| THE CAMPAIGN'S POLICY: a map built and stood up, which map a measured run is on (`pinnedFor`), the points, the locks, the table, the board; a rung booked at the flag and the plate's lines | `pwa/src/game/campaign.ts` (storage-free above its line), `campaign-run.ts`, `pinned-run.ts` | `campaign` |
| The campaign card and the level card, the loop on each box | `pwa/src/game/menu-campaign.tsx`, `menu-levels.tsx`, `menu-pinned.tsx`, `route-shape.ts`, `strings-campaign.ts`, `pwa/src/campaign.css` | `campaign`, `menu-system` |
...
| THE WEATHER a map is dealt (R19) and its evening; a sky chosen by hand (`withSky`); the moon (`moonAtRun`); the wind as a pure field (`windAt`) | `engine/mapgen/weather.ts`, `engine/game/clock.ts`, `engine/game/wind.ts` | `atmosphere`, `mapgen-improvement` |
...
| The weather as drawn: the cloud, the stars and the moon on the dome, the falling snow and the spindrift, the lamps' glow and their beam on the snow | `sky-dome.ts`, `snowfall.ts`, `sled-body.ts`'s `lampMounts`, `snow-glsl.ts`'s lamp and flat-light terms | `atmosphere`, `snow-look` |
...
| THE SCORE AS READ: the combo over the nose, the score and the buzzer, the TRICK press; the trick vocabulary | `pwa/src/game/trick-tile.ts` (DOM-free), `hud-combo.tsx`; `TRICK_WORDS` / `comboLine` in `strings.ts` | `hud-and-menus` |
| THE RECORD BOOK: the best time per seed, sled, mode and laps, its crossings, what beats a row | `pwa/src/game/records.ts` (storage-free above its storage line; `tests/records_test.ts`) | `menu-system`, `hud-and-menus` |
| THE GHOST: a run kept as the controls that rode it (the tape, its grid, what names the map), the rig that files every run and steps the ghost beside the player's, the ghost as drawn | `pwa/src/game/ghost.ts` (pure above its storage line), `ghost-run.ts`, `ghost-model.ts`; `tests/ghost_test.ts` | `engine-system`, `menu-system` |
| THE REPLAY: any race or time trial rebuilt — the field with it — and ridden off its tape INSTEAD of you; the rig the app drives; the bar | `pwa/src/game/replay.ts` (the tape, the recipe, the rebuild — DOM-free), `replay-run.ts` (the rig, `WATCHING_CAMERAS`), `hud-replay.tsx`; `tests/replay_test.ts` | `menu-system`, `engine-system` |
| THE CLOUD SAVE: the book, the ghosts, the board and the rider's half of the settings carried between the rider's devices (the store app, iOS) — what travels and how two devices merge; when it pulls and pushes | `pwa/src/game/cloud-save.ts` (pure above its storage line; `tests/cloud_save_test.ts`), `use-cloud-sync.ts`; the shell half `native/src/cloud-ask.ts`, `cloud-save.ts`, `native/modules/cloud-save/` | `platform-shells`, `menu-system` |
| WHAT A REPLAY CUTS TO AND HOW SLOWLY: the moments, the running order, the director | `pwa/src/game/replay-shots.ts` (three-free: `createShotCollector`, `planShots`, `directAt`) | `menu-system`, `game-feel` |
| THE BROADCAST: the lens planted at the edge of the wood for a moment the recording knows is coming — replay-only | `pwa/src/game/camera-tv.ts` (three-free), applied by `renderer.setShot` | `game-feel` |
...
| The cards: attract, front door, loading, pause (and which figures a held race is billed with — `pause-stats.ts`, DOM-free), and walking them on the keys | `pwa/src/game/splash-screen.tsx`, `splash.ts`, `menu-main.tsx`, `loading-screen.tsx`, `menu-pause.tsx`, `pause-stats.ts`, `menu-nav.ts`, `menu-cursor.ts`, `menu-glyphs.tsx`, `mark-trails.tsx` | `menu-system` |
| Standing a race up behind a card | `pwa/src/game/run-loader.ts` (the sequencing), `app-load.ts` (the steps) | `menu-system` |
| THE START CARD: the free ride's map, date, hour, weather (`sky.weather` only — the hour is `day`'s) and snow, and the chart a spot is picked on | `pwa/src/game/menu-start.tsx`, `free-ride.ts` (DOM-free: what is stored, `freeGameOptions`), `seed-preview.tsx` over `seed-preview-worker.ts`, `seed-chart.ts` (DOM-free) | `menu-system` |
| ONE OF THE GAME'S OWN BUTTONS, wherever the press came from | `pwa/src/game/run-actions.ts` | `menu-system`, `hud-and-menus` |
| THE SHUTTER: a picture asked for at the press and served at the frame, what is decided about it (size, name, stamp, the HUD layer as a document), the HUD rasterized in, the caption (`run-news.ts`'s `shotLabel`) | `pwa/src/game/shot-request.ts`, `screenshots.ts`, `shot-plan.ts` (DOM-free), `shot-hud.ts`; words in `strings-gallery.ts` | `menu-system`, `hud-and-menus` |
| THE ROLL AND THE GALLERY: the policy (newest first, forty kept, storage-free), the IndexedDB store, the thumbnails, the viewer, the three ways a picture leaves | `pwa/src/lib/shot-roll.ts`, `shot-store.ts`, `shot-thumbs.ts`, `share-image.ts`; `pwa/src/game/menu-gallery.tsx`; `tests/screenshots_test.ts` | `menu-system`, `ui-review` |
...
- **WHAT THE RULES ARE** — `engine/mapgen/rules.ts`: R1–R21 stated once in the header as prose and every number in `LEVEL_RULES`. The generator BUILDS to them, `engine/analysis/` HOLDS the finished map to them, the tests assert them, and `docs/level-generator.md` carries the prose verbatim.
- **WHICH GENERATOR BUILT A MAP** — `engine/mapgen/versions.ts`. `CURRENT_GENERATOR_VERSION` is the rules in this tree and is what everything but a campaign map gets; a campaign map names its own (`CampaignLevel.version`, written out on every map on purpose) and carries the DIGEST of the map that came out (`levelDigest`), which `tests/generator_version_test.ts` rebuilds and compares. A change that moves what a seed builds owes a version row with the old behaviour kept as a trait — never a regenerated digest, which is the silent re-roll the scheme exists to prevent.
- **WHICH MAP A MEASURED RUN IS ON** — `pinnedFor` in `pwa/src/game/campaign.ts`: a RACE and a TIME TRIAL ride the campaign map the level card picked (`Settings.level`), a FREE RIDE its own seed, and a link's `?seed=` takes the pinned map off for that visit. A campaign RUNG is booked on its board by `campaign-run.ts`'s rig and filed in the record book like any other measured run.
...
- **The weather** — `Level.weather` (R19, `engine/mapgen/weather.ts`), always read through `weatherOf` (a hand-built map is under `CLEAR_WEATHER`); a sky or an hour chosen by hand goes through `withSky` (`GenerateOptions.sky`, `CreateGameOptions.sky`, `?weather=` / `?hour=`) and moves nothing the map builds. The wind is `windAt(level, t)` — a pure function, drawing nothing from `state.rng`, read by the picture and by nothing in the physics. `skyLookAt` is the one place the weather and the night become a picture: the flat light, the glitter, the lamps, the fall, the fog all come off that `SkyLook`.
...
- **Which surface is up** — `Shell` in `pwa/src/game/shell.ts` (`splash | menu | loading | pause | run | replay`), and the predicates beside it (`watching` is the replay's). **The snow never stops behind a card — except the pause card**, the one surface `simulates` says no to.
...
- **What the game remembers** — `pwa/src/game/settings.ts` (the camera, the sled picked, the sound switch, and every OPTIONS row: the faders, the picture, the keys, the thumbs, the assist; the time trial's length), and `mergeSettings` is the one place a stored blob becomes settings this build offers — the record book (`records.ts`), the ghosts (`ghost.ts`) and the campaign's board (`campaign.ts`) are the only other things kept in `localStorage`, each under its own key and each checked the same way on the way in (plus the cloud save's one stamp, `cloud-save.ts`'s `CLOUD_KEY` — when this device last moved a carried setting); the SCREENSHOT ROLL is IndexedDB (`lib/shot-store.ts`, policy in `lib/shot-roll.ts`) and never load-bearing. `mixOf` is the one place the master and the switch fold into the two faders the bus is handed.
...
- **WHAT A TRICK IS WORTH** — `engine/game/tricks.ts` (and `TUNING.tricks`): the air by the second and by the metre on one curve (tied at `lengthKnee`), the Nth revolution on an axis N × its points and N steps of multiplier, the twist, the poses, the air's own rung sold only beside a trick, and the combo banked on a clean landing, at its base on a sketchy one, and lost to a wipeout, a reset or a landing in a pose. What a stroke turns is `strokes.ts`'s: all or nothing at the top of an axis, from a per-flight budget, never opened on the way down. No WORD for any of it is in the engine — `TRICK_WORDS` / `comboLine` in `strings.ts`. The score is kept on every run (the sim reads it) and worked for only where `rules.tricks` is on.
- **What a run is playing by** — `GameState.rules` (`RunRules`), dealt by `createGame` from its mode's bundle in `MODE_RULES` (`engine/game/defs/modes.ts`). A mode is a NAME the app uses to pick a card and a row of the record book; nothing below the app branches on it.
- **WHAT A RUN WAS RIDDEN ON, TO THE FIGURE** — `snapInput` in `pwa/src/game/ghost.ts`, applied where the player's input is made (`input-model.ts`'s `sampleInput`) and again where `App.tsx` hands the engine any input at all: every axis on the tape's grid (1/127 of lock and lean, 1/255 of a lever), so the figure the engine is ridden on IS the figure a ghost records and the ghost is the run again rather than nearly it. `ControlTape`, `createControlRecorder` and `readControls` beside it are the only encoder and decoder of a tape.
...
- **Which shell the page is in** — `pwa/src/shell-host.ts`: the frozen `__SH_SHELL__` global and the `sh-` events (`SHELL_RUMBLE`, `SHELL_COMMAND` and its word list, the fullscreen pair, the cloud's ask and answer `SHELL_CLOUD` / `SHELL_CLOUD_EVENT`). The ONE line of `pwa/` that knows a shell exists.
...
| A campaign map (`campaign-levels.ts`) | `make rate CAMPAIGN=1` and `make difficulty CAMPAIGN=1` (both in the PR), `make routes`, the digest rewritten only for a map deliberately moved; `docs/getting-started.md`'s campaign paragraph if a name changed |
...
- **The rule book has a mirror.** `engine/mapgen/rules.ts` states R1–R21 once in its header; `docs/level-generator.md` carries the same prose VERBATIM. `tests/docs_rules_test.ts` reads the ids off the code, so a new rule fails the test until its mirror lands.
...
- **THE SHUTTER'S WORD IS NAMED FOUR TIMES.** `shot` is in `SHELL_COMMANDS` (`pwa/src/shell-host.ts`), a row of `tauri/shell/src/menu.rs` (View ▸ Take Screenshot) and the `SHOT_COMMAND` script the store app injects when the phone takes a screenshot (`native/src/injected.ts`, fired from `native/src/screen-capture.ts`); `tests/tauri_test.ts` and `tests/shell_test.ts` hold them, and `run-actions.ts` lands it on the ENTER key's line.
- **THE CLOUD SAVE IS NAMED THREE TIMES.** The ask and its answer (`SHELL_CLOUD` / `SHELL_CLOUD_EVENT` in `pwa/src/shell-host.ts`), the listener that posts an ask (`CLOUD_BRIDGE` in `native/src/injected.ts`) and the parser and the answer's script (`parseCloudAsk` / `cloudReply` in the import-free `native/src/cloud-ask.ts`) cannot import each other; `tests/shell_test.ts` holds all of it by running the scripts. WHAT is saved and how two devices merge is the website's (`pwa/src/game/cloud-save.ts`, every rule the game's own — `beats`, the board's better afternoon — imported, never restated); the shell moves one opaque string.
...
- **`add-region`** — a NEW KIND OF SNOW COUNTRY end to end, in order (R21): the engine's row, the surface the physics reads, the look and the grade, the card's word, the suite, the labs and the docs — with the boreal row kept all ones so no seed moves.
...
- **`level-rating`** — whether a generated map is any GOOD and how HARD: `engine/rating/`'s eight axes and the index, the bands calibrated off a sweep (`make rate ARGS=--stats`), the ladder scorer, and `make difficulty`'s schematic.
- **`campaign`** — the pinned maps and the ladder they make (`campaign-levels.ts`, `campaign.ts`, the cards): curating a rung with `make rate CAMPAIGN=1` and the bot, the generator-version contract a pinned map stands on, the points and the locks, and which map a measured run is on.
...
**Reserved, not written** (see `.agents/skills/README.md`): soundtrack, tricks, debug-tools, replay, store-listing, store-shots. A lesson about one of those subjects waits, scoped, in the nearest existing skill until its subject is built — and the sibling repos' skill of the same name is where it starts.
