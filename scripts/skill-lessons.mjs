...
function parseFragment(file) {
  ...
  else if (!DATE_RE.test(meta.date)) errors.push(`date "${meta.date}" is not YYYY-MM-DD`);
  ...
  for (const c of concepts) {
    if (!CONCEPT_RE.test(c)) errors.push(`concept "${c}" must be lower-case kebab (a-z0-9-)`);
  }
...
const conceptFilter = splitList(flagValue("concepts")).map((c) => c.toLowerCase());
const fileFilter = splitList(flagValue("files"));
const filtering = scopeFilter.length > 0 || conceptFilter.length > 0 || fileFilter.length > 0;
const skill = args.find((a) => !a.startsWith("--"));

if (skill && !existsSync(path.join(SKILLS_DIR, skill, "SKILL.md"))) {
  console.error(`Unknown skill "${skill}". Available: ${allSkills().join(", ")}`);
  process.exit(1);
}
...
function matches(lesson) {
  if (fileFilter.length && !fileFilter.some((f) => lesson.name.includes(f))) return false;
  if (conceptFilter.length && !lesson.concepts.some((c) => conceptFilter.includes(c.toLowerCase())))
    return false;
  if (scopeFilter.length) {
    ...
    if (!lesson.scope.some((s) => scopeFilter.some((q) => pathsOverlap(s, q)))) return false;
  }
  return true;
}
...
function printIndex(skillName, lessons) {
  console.log(`# ${skillName} — ${lessons.length} lesson(s)${describeFilter()}\n`);
...
function printBodies(skillName, lessons) {
  console.log(`# Lessons learned — ${skillName} (${lessons.length})${describeFilter()}\n`);
  for (const l of lessons) {
...
if (checking) {
  ...
  if (!soft) console.log("All lesson fragments are well-formed and within the size bars.");
} else if (vocab) {
  ...
  if (!counts.size) {
    ...
    for (const [c, n] of [...counts].sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0]))) {
