...
| `scripts/sled-preview.mjs`, `pwa/src/tools/sled-harness.ts` | `make sled` — THE SLED LAB: contact sheets built with the game's own builder. `asset` (`--asset=a.glb`: a machine MODELLED in Blender beside the builder's — the `blender-assets` skill), `machines` (every machine by side, front, rear, three-quarter and chase — the elevations orthographic on a metre grid), `liveries`, `poses` (one machine, every pose the rider takes), `landing` (the body on its legs through a landing, a frame every 60 ms). No `make build` |
