...
| `pwa/src/game/posed-merge.ts` | The posed tree — machine and rider — drawn as ONE mesh, every part a rigid BONE of it, so the GPU lays each part through its matrix (in every pass, the shadows included); a part hidden with its ancestors (the rider in the cockpit views) collapses to nothing |
