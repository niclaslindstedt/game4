...
This repository was bootstrapped with every `.lessons/` directory EMPTY (a
`.gitkeep` and nothing else). That is not a sign the tool is broken — it is the
state every skill starts in, and the first sessions through each skill are the
ones that fill it. A skill whose lessons are empty is read from its `SKILL.md`
alone; reflect into it anyway.
...
one. A session that loaded `sled-physics` and `simulate-run` owes two passes.
...
date: YYYY-MM-DD
scope: engine/game/snow.ts, engine/game/defs/sled.ts
concepts: [sink, probes, powder]
---
...
| --- | --- | --- |
| `title` | yes | The gist in one line. Written as a CLAIM, not a topic — "The bump stop must read the probe's OWN closing speed, not the CoG's, or a flat landing bounces", never "About landings". |
...
- The filename timestamp is `$(date +%s)` at recording time (same scheme as
  ...
  not an error, because it is the staleness signal.
...
script. A lesson scoped `engine/` is a lesson that should have had no scope at
all. And a scope pointing at a path that no longer exists is the single best
...
```sh
cat > .agents/skills/<skill>/.lessons/$(date +%s)-short-slug.md <<'LESSON'
---
title: …
date: YYYY-MM-DD
scope: …
...
is wrong" next to a step 4 that is still wrong. This matters doubly in a
freshly bootstrapped repo: the skills were written against a blueprint before
most of the code existed, so a path or a flag that turns out not to match what
was built is fixed in the skill, the same PR, without ceremony.
...
concepts (`kicker`/`kickers`, `sim`/`simulation`) get merged in the same pass.
...
checklist item — then **delete the fragment**. A promoted lesson lives in
exactly one place.
...
`AGENTS.md` (the file `CLAUDE.md` and friends symlink to) is read at the start
of **every** session, whatever the task. That makes it the most expensive text
in the repo, and it is under constant pressure to grow: every rule learned the
hard way wants to live there.
...
create one when no existing skill owns the subject. `.agents/skills/README.md`
lists the skills that exist and the ones RESERVED for subjects not yet built
(sound, atmosphere, damage, the platform shells…); a lesson about a reserved
subject waits in the nearest existing skill, scoped, until that skill is
written — and writing that skill is the moment those fragments move.
